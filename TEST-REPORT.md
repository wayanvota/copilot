...
The reviewed release is now deployed. The Render backend runs from `main` at commit `c7c352c`, and the rebuilt static frontend is live at `https://wayan.com/copilot/`. The release passed the full automated regression suite, a clean production build, dependency audits, browser checks, safe attack simulations, repeated OpenAI-backed questions, production security-header checks, protected-route checks, concurrent production requests, and a producer question submitted through the public Firefox interface.

The remaining limitations are substantive but do not block a controlled producer beta. Manure-pit entry procedures remain an evidence gap, Safari/WebKit and a real screen reader remain untested, the rate limiter is process-local, and no public data-retention/deletion policy has been adopted. This review does not establish that the application is secure or that every answer is legally complete. It establishes what resisted the tests described here and confirms that the public deployment matches the reviewed release.
...
| Deployed backend commit | `c7c352c` on `main` |
| Local working tree | Pre-existing user collateral and unrelated PDFs preserved; deployment evidence and final report updates remain local at the time of this entry |
...
| Browsers exercised | Chrome, Firefox, and the Codex in-app Chromium browser |
...
- **5 material subchecks remain unverified**, principally Safari/WebKit, a real screen reader, automated color-contrast measurement, retention/deletion policy, and destructive or load testing that was intentionally excluded.

Automated totals after fixes were **36 backend tests passed** and **8 frontend tests passed**. Five direct AI adversarial cases passed. Five representative producer questions were repeated against production. Four returned verified evidence with 6 to 7 citations each. The manure-pit entry question correctly returned insufficient evidence with two supporting citations rather than inventing a complete procedure. Four simultaneous production questions completed successfully. The frontend and Python dependency audits reported zero known vulnerabilities after the installer was upgraded and the reviewed dependency set was refreshed.
...
| 8 | Primary user flows | Pass | Five realistic end-to-end questions covered permits, manure acreage, non-family youth labor, African Swine Fever signs, and manure-pit safety. Four returned verified evidence. The manure-pit question visibly abstained from claiming a complete entry procedure because the corpus did not support one. |
...
| 12 | Routing and direct entry | Pass after deployment fix | `/copilot/`, `/copilot/admin/`, robots, sitemap, and static assets returned 200. An unknown live route returned the copilot-specific 404 artifact with status 404 and the frontend security headers. A child-directory rewrite rule was added after the first live check exposed the parent WordPress 404. |
...
| 15 | Concurrency and idempotency | Pass after fix | Duplicate feedback and bookmarks previously created duplicate rows; both now upsert. Three concurrent local questions initially produced two SQLite lock failures. The write transaction is now committed before retrieval/model work. The automated regression, repeated local run, and four simultaneous production questions passed. |
...
| 18 | Cross-browser compatibility | Partial pass | The public homepage rendered correctly in Chrome, Firefox, and the in-app Chromium browser. A complete manure question was submitted and rendered in Firefox. Safari/WebKit remains unverified. |
...
| 26 | Slow network and caching | Pass with limitation | Rejected and interrupted fetches were simulated in component tests; the UI recovered. The live Apache deployment sends no-cache behavior for HTML and long-lived rules for static asset types. Offline mode and service workers are not product features. |
...
| 35 | CORS and security headers | Pass after deployment | The live backend and frontend send CSP, frame denial, MIME-sniffing protection, referrer policy, permissions policy, and HSTS. The frontend CSP restricts API connections to the deployed Render origin. Protected conversation access rejected a missing or invalid administrator key. |
...
| 47 | Live-site verification | Pass with limitations | The public frontend and backend were checked after deployment. Health returned 200 with Nebraska, 51 documents, and `gpt-5.6-terra`. The source endpoint returned 51 records and zero Iowa mentions. Five production POSTs exercised permits, manure, labor, animal health, and safety; four simultaneous requests completed; a protected conversation request returned 401; Firefox completed a public manure question; and the live headers, robots, sitemap, admin entry, assets, production API URL, and application 404 were verified. No destructive administrator mutation or load test was run. |
...
| D4 | Medium | Frontend and some backend responses lacked a complete CSP, anti-framing policy, HSTS, and application 404 behavior. | Added API middleware headers and FTP Apache header/cache/404 rules. | Header assertions pass locally and on both public services; an unknown public route returns the copilot 404 with status 404. |
...
The unprotected conversation-history endpoint was a confirmed authorization weakness in the pre-review source. The corrected deployed endpoint now rejects requests without the administrator key. Dependency audits also confirmed known vulnerable versions in both package ecosystems before the upgrades.

### Risks mitigated in the local candidate

The deployed release adds administrator authorization, request-size enforcement, safer rate-limit identity handling, idempotent mutations, stricter input validation, CSP and anti-framing headers, production readiness checks, network-error normalization, citation-artifact sanitization, and shorter database write transactions. Tested XSS, injection, SSRF, traversal, redirect, hostile CORS, fabricated citation, and prompt-injection payloads did not bypass the tested controls.

### Areas outside the evidence

No destructive payloads, credential attacks, denial-of-service, aggressive load, third-party scans, or destructive production mutations were run. Render deployment settings and live Apache behavior were inspected, but Neon administration, DNS administration, backups, alerting, and OpenAI account settings were not audited. The review did not establish data retention, deletion, legal compliance, incident response, disaster recovery, multi-instance rate limiting, or corpus legal accuracy. It did not include Safari/WebKit, a real screen reader, or formal performance lab instrumentation.
...
- `06-local-tablet.png`
- `07-local-large.png`
- `08-live-production-manure-answer.png`
...
The deployed release clears the technical gate for a controlled producer beta. Before describing it as a production compliance service, complete four remaining actions:

1. Add an authoritative manure-pit confined-space entry source or keep the current visible abstention for that question.
2. Run Safari/WebKit, VoiceOver, and rendered color-contrast checks.
3. Replace the process-local rate limiter if Render is scaled to multiple application instances.
4. Publish a short data-retention and deletion policy before inviting producers to enter operational details.

The current evidence supports **READY WITH KNOWN LIMITATIONS** for a controlled producer beta, with citations and abstentions still requiring human review.
