...
def test_chunk_ids_are_removed_from_visible_claim_text():
    ...
    assert (
        _clean_claim_text(f"A cited rule applies [{chunk_id}].")
        == "A cited rule applies."
    )
    leaked = (
...
def test_internal_source_aliases_are_removed_from_all_visible_answer_fields():
    answer = answer_with("S1")
    answer.missing_facts = ["A manure analysis is needed.【S8】"]
    answer.limitations = ["County rules were not retrieved [S2]."]
    answer.related_questions = ["What is the storage type? 【S3】"]
    answer.applicability.explanation = "More farm facts are required.【S4】"

    checked = _validate_citations(answer, {"S1": "retrieved"})

    assert checked.missing_facts == ["A manure analysis is needed."]
    assert checked.limitations == ["County rules were not retrieved."]
    assert checked.related_questions == ["What is the storage type?"]
    assert checked.applicability.explanation == "More farm facts are required."


def test_farm_context_is_explicitly_labeled_as_user_context():
    context = FarmContext(
        county="Madison", operation_type="confinement", animal_unit_capacity=1000
    )
...
def test_approved_source_hosts_only():
    assert _allowed_url(
        "https://dwee.nebraska.gov/land-waste/agriculture/livestock-waste-control-program"
    )
...
def test_every_registered_manual_pdf_has_extractable_text():
    manual_sources = [
        source for source in APPROVED_SOURCES if source.get("manual_path")
    ]
...
def test_manual_source_replaces_existing_chunks(tmp_path, monkeypatch):
    ...
    source = next(
        item for item in APPROVED_SOURCES if item["title"].startswith("Title 130")
    )
    monkeypatch.setattr(
        "app.ingest.embed_texts", lambda texts: [[0.0] * 1024 for _ in texts]
    )
    with Session() as db:
        document = Document(
            title=source["title"],
...
def test_nebraska_query_expansion_uses_canonical_terms():
    ...
    construction, _ = _expand_query(
        "What separation distances apply to a new Nebraska hog barn near a well?"
    )
...
def test_manure_acreage_query_prioritizes_land_requirement_evidence(
    ...
    Base.metadata.create_all(engine)
    monkeypatch.setattr(
        "app.retrieval.embed_texts", lambda texts: [[0.0] * 1024 for _ in texts]
    )
    with Session() as db:
        land = Document(
            title="Manure Nutrient Production and Land Requirements",
            agency="Nebraska Extension",
        ...
        other = Document(
            ...
            url="https://dwee.nebraska.gov/example",
            jurisdiction="Nebraska",
            topic="manure",
            source_tier=2,
            document_type="guidance",
        ...
        db.add_all(
            [
                Chunk(
                    document_id=land.id,
                    ordinal=0,
                    content="Calculate sufficient acres from annual manure nutrients, planned application rate, crop needs, and soil tests.",
                    token_count=20,
                    embedding=[0.0] * 1024,
                ),
                Chunk(
                    ...
                    ordinal=0,
                    content="Livestock waste facilities require inspection.",
                    token_count=8,
                    embedding=[0.0] * 1024,
                ),
            ]
        ...
        results = hybrid_search(
            db,
            "How much land do I need to spread manure from my pigs?",
            [2],
            ["manure"],
        )
